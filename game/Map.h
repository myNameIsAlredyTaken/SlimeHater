...
bool Level::LoadFromFile(std::string filename)//двоеточия-обращение к методам класса вне класса 
{
	...
	std::string path = tilesetElement->Attribute("source");
	TiXmlDocument xmlFile(path.c_str());

	if (!xmlFile.LoadFile())//если не удалось загрузить карту
	{
		std::cout << "Loading level \"" << path << "\" failed." << std::endl;//выдаем ошибку
		return false;
	}

	TiXmlElement* xmlTmp;
	xmlTmp = xmlFile.FirstChildElement("tileset");

	TiXmlElement* image = xmlTmp->FirstChildElement("image");
	...
	while (layerElement)
	{
		...
		while (tileElement)
		{
			std::string id = tileElement->Attribute("gid");
			int tileGID = atoi(id.c_str());
